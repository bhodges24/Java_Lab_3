...
public class Deck {
	...
	public Deck(int num_jokers) {
		...
		for (int i = 1; i <= num_jokers; i++) {
			cards.add(new Card(eSuit.JOKER, eRank.JOKER, 52 + i));
		}
		ShuffleCards();
	}

	public Deck(int num_jokers, ArrayList<Card> WildCards){
		//Call 1-Arg Constructor to add Jokers to deck
		this(num_jokers);
		
		//Loop over WildCards and the Cards in Deck
		for(Card card : cards){
			for(Card wild : WildCards){
				/*If the card in the deck is the same as the wild  
				*card, make the card in the deck a wild card */
				if (wild.getRank() == card.getRank() &&
					wild.getSuit() == card.getSuit()){
					card.setWild();
				}
			}
		}
